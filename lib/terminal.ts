...
import type { GhosttyKittyImagePlacement, ILink, ILinkProvider } from './types';
...
type TerminalWriteOperation =
  | { kind: 'wasm'; data: string | Uint8Array }
...
export class Terminal implements ITerminalCore {
  ...
  private syntheticResponseQueue: string[] = [];
  ...
  constructor(options: ITerminalOptions = {}) {
    ...
    const baseOptions = {
      ...
      kittyImageStorageLimit: options.kittyImageStorageLimit,
  ...
  private handleOptionChange(key: string, newValue: any, oldValue: any): void {
    ...
    switch (key) {
      ...
      case 'kittyImageStorageLimit':
        if (this.wasmTerm && typeof newValue === 'number') {
          this.wasmTerm.setKittyImageStorageLimit(newValue);
  ...
  private handleFontChange(): void {
    ...
    this.wasmTerm.resize(this.cols, this.rows, metrics.width, metrics.height);
  ...
  private buildWasmConfig(): GhosttyTerminalConfig | undefined {
    ...
    return {
      ...
      palette,
      kittyImageStorageLimit: this.options.kittyImageStorageLimit,
    };
  }
  ...
  open(parent: HTMLElement): void {
    ...
    try {
      ...
      this.renderer.resize(this.cols, this.rows);
      const initialMetrics = this.renderer.getMetrics();
      this.wasmTerm.resize(this.cols, this.rows, initialMetrics.width, initialMetrics.height);
  ...
  private writeInternal(data: string | Uint8Array, callback?: () => void): void {
    ...
    for (const operation of this.buildWriteOperations(data)) {
      if (operation.kind === 'wasm') {
        const hasContent =
          typeof operation.data === 'string'
            ? operation.data.length > 0
            : operation.data.length > 0;
        if (hasContent) {
          this.wasmTerm!.write(operation.data);
        }
      } else {
        this.syntheticResponseQueue.push(operation.data);
      }

      // Drain any side effects generated by this step so responses preserve input order.
      this.processTerminalEffects();
    }
  ...
  paste(data: string): void {
    ...
    const encoded = this.wasmTerm!.encodePaste(data, this.wasmTerm!.hasBracketedPaste());
    if (encoded.length > 0) {
  ...
  resize(cols: number, rows: number): void {
    ...
    try {
      ...
      // Resize WASM terminal using browser cell metrics for kitty sizing.
      this.wasmTerm!.resize(cols, rows, metrics.width, metrics.height);
  ...
  public formatSelection(
    format: GhosttyFormatterFormat = GhosttyFormatterFormat.PLAIN,
    options: Omit<GhosttyFormatterOptions, 'format' | 'selection'> = {}
  ): string {
    if (!this.wasmTerm) {
      throw new Error('Terminal not open');
    }

    const selection = this.selectionManager?.getAbsoluteSelectionRange();
    if (!selection) {
      return '';
    }

    return this.wasmTerm.format({
      ...options,
      format,
      selection,
    });
  ...
  public getKittyGraphicsPlacements(): readonly GhosttyKittyImagePlacement[] {
    if (!this.wasmTerm) {
      throw new Error('Terminal not open');
    }

    return this.wasmTerm.getKittyGraphicsPlacements();
  }
  ...
  private processTerminalResponses(): void {
    ...
    while (this.syntheticResponseQueue.length > 0) {
      const response = this.syntheticResponseQueue.shift();
      if (response) {
  ...
  private processTerminalEffects(): void {
    this.processTerminalResponses();
    this.processBellEvents();
    this.processTitleChanges();
  }

  private processBellEvents(): void {
    if (!this.wasmTerm) return;

    const bellCount = this.wasmTerm.readBellCount();
    for (let i = 0; i < bellCount; i++) {
      this.bellEmitter.fire();
    }
  }

  private processTitleChanges(): void {
    if (!this.wasmTerm) return;

    while (true) {
      const title = this.wasmTerm.readTitleChange();
      if (title === null) break;
  ...
  private buildWriteOperations(data: string | Uint8Array): TerminalWriteOperation[] {
    const bytes = typeof data === 'string' ? textEncoder.encode(data) : data;
    const operations: TerminalWriteOperation[] = [];
    let cursor = 0;
    let i = 0;
    let foundOscQuery = false;

    while (i < bytes.length - 1) {
      if (bytes[i] !== ESC || bytes[i + 1] !== OSC) {
        i++;
        continue;
      }

      const terminator = this.findOscTerminator(bytes, i + 2);
      if (!terminator) {
        break;
      }

      const payload = textDecoder.decode(bytes.slice(i + 2, terminator.contentEnd));
      const response = this.buildOscColorQueryResponse(payload);
      if (!response) {
        i = terminator.nextIndex;
        continue;
      }
      ...
      if (cursor < i) {
        operations.push({
          kind: 'wasm',
          data: this.sliceWriteData(bytes, cursor, i, typeof data === 'string'),
        });
      }
      operations.push({ kind: 'response', data: response });
      cursor = terminator.nextIndex;
      i = terminator.nextIndex;
    }

    if (!foundOscQuery) {
      return [{ kind: 'wasm', data }];
    }

    if (cursor < bytes.length) {
      operations.push({
        kind: 'wasm',
        data: this.sliceWriteData(bytes, cursor, bytes.length, typeof data === 'string'),
      });
    }
  ...
  private findOscTerminator(
    bytes: Uint8Array,
    start: number
  ): { contentEnd: number; nextIndex: number } | null {
    for (let i = start; i < bytes.length; i++) {
      if (bytes[i] === BEL) {
        return { contentEnd: i, nextIndex: i + 1 };
      }
      if (bytes[i] === ESC && i + 1 < bytes.length && bytes[i + 1] === ST) {
        return { contentEnd: i, nextIndex: i + 2 };
      }
    }
    return null;
  }

  private sliceWriteData(
    bytes: Uint8Array,
    start: number,
    end: number,
    asString: boolean
    ...
    return asString ? textDecoder.decode(slice) : slice;
  }

  private buildOscColorQueryResponse(payload: string): string | null {
    if (payload === '10;?') {
      return `\x1b]10;${this.formatOscThemeColor('foreground')}\x1b\\`;
    }

    if (payload === '11;?') {
      return `\x1b]11;${this.formatOscThemeColor('background')}\x1b\\`;
    }

    if (payload === '12;?') {
      return `\x1b]12;${this.formatOscCursorColor()}\x1b\\`;
    }

    return null;
  }

  private formatOscThemeColor(colorKey: 'foreground' | 'background'): string {
    const value = this.parseThemeColorToHex(this.options.theme[colorKey], DEFAULT_THEME[colorKey]);
    const r = (value >> 16) & 0xff;
    const g = (value >> 8) & 0xff;
    const b = value & 0xff;

    return `rgb:${this.expandColorChannel(r)}/${this.expandColorChannel(g)}/${this.expandColorChannel(b)}`;
  }

  private formatOscCursorColor(): string {
    const dynamicCursorColor = this.wasmTerm?.getDynamicCursorColor();
    const value = this.parseThemeColorToHex(
      dynamicCursorColor ?? this.options.theme.cursor,
  ...
  private parseThemeColorToHex(color: string | undefined, fallback: string): number {
    if (color) {
      const parsed = this.parseColorToHex(color);
      if (parsed !== 0 || this.isZeroRgbColor(color)) {
        return parsed;
      }
    }

    return this.parseColorToHex(fallback);
  }

  private isZeroRgbColor(color: string): boolean {
    return /^#(?:000|000000)$/i.test(color) || /^rgb\(\s*0\s*,\s*0\s*,\s*0\s*\)$/i.test(color);
  }

  private expandColorChannel(channel: number): string {
